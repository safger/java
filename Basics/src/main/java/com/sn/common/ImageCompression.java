 ...
 */
 public class ImageCompression { 
	 ...
	 public String compressPic() { 
		 try { 
			 ...
			 if (img.getWidth(null) == -1) {
				 ...
				 //判断原图大小与压缩大小
				 if((double) img.getWidth(null)<=outputWidth){
 	...
 	public static void main(String[] arg) { 
 		...
 		System.out.println("输入的图片大小：" + mypic.getPicSize("/upload/20121261838397.jpg")/1024 + "KB"); 
 		int count = 0; // 记录全部图片压缩所用时间
 		for (int i = 0; i < 2; i++) { 
 			int start = (int) System.currentTimeMillis();	// 开始时间 
 			mypic.compressPic("/Street/upload", "/Street/upload", "20121261838397.jpg", "r1"+i+".jpg", 240, 240, true); 
