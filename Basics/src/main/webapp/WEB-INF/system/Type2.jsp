	...
	<head>
	 ...
	 jQuery(document).ready(function($) {
		  $("#page").AutoPage({
					...
					url:'<%=basePath%>system/data/showChild2.html?parentcode=${parentcode}&skey=${skey}',
					showNum : 6  // 显示数字
					,show:1
				});
	 });
	 function serach(){
	 var skey=$("#mc").val();
	 var url="<%=basePath%>system/data/showChild2.html?skey="+skey+"&parentcode="+parentcode;
				...
				<table width="100%" cellSpacing="0" cellPadding="0" class="table">
       			 <tr class="title">
						...
						<c:if test="${com.hisDelete==true }">
							...
							<a href="<%=basePath%>system/data/showChild3.html?parentcode=${list.code }" class="blue-12" >编辑子目录</a>
						</td>
					</tr>
						</c:forEach>
					 </table>
